...
class IntervalTest(unittest.TestCase):
    ...
    def testGetItemNegative(self):
        ...
        iv = next(ivf)
        self.assertTrue(
            iv[-self.fieldcount + self.chrpos].startswith("chr"),
            iv[-self.fieldcount + self.chrpos],
        )
        self.assertTrue(
            iv[-self.fieldcount + self.startpos].isdigit(),
...
def test_missing_files():
    ...
    bedFile.cpp
    """
    a = pybedtools.BedTool("chrA 1 10", from_string=True).saveas(
        "this_file_should_raise_BEDTools_Error"
    )
    result = list(iter(a))
    os.unlink(a.fn)
